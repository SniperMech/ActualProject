...
public class DBConnection {
	...
	public static String[] getGenreIDs() throws SQLException{
		pstmt=conn.prepareStatement("SELECT GenreID FROM gggameshopv2v2.genre");
		rs=pstmt.executeQuery();
	...
	public static String[] getLogins(String Column) throws SQLException{
		pstmt=conn.prepareStatement("SELECT "+Column+" FROM gggameshopv2v2.logins");
		rs=pstmt.executeQuery();
	...
	public static String[] getValues(String Column) throws SQLException{
		pstmt=conn.prepareStatement("SELECT "+Column+" FROM gggameshopv2v2.games");
		rs=pstmt.executeQuery();
	...
	public static String[] getSpecificGames(String Column, String preowned, String genre, String search) throws SQLException{
		if(search!=""){
			pstmt=conn.prepareStatement("SELECT "+Column+" FROM gggameshopv2v2.games Ga WHERE Title=?");
			...
			if(genre!=""){
				...
				pstmt=conn.prepareStatement("SELECT "+Column+" FROM gggameshopv2v2.games Ga WHERE Preowned=?");
			...
			pstmt=conn.prepareStatement("SELECT "+Column+" FROM gggameshopv2v2.games Ga");
		}
		rs=pstmt.executeQuery();
	...
	public static void insertProduct(String Title, String Company, String Date,
		...
		pstmt=conn.prepareStatement("insert into gggameshopv2v2.games(GameID, Title, Company, Date, Description, Price,"
		...
		for(int i=0;i<Genres.length;i++){
			pstmt=conn.prepareStatement("insert into gggameshopv2v2.gamegenre(GameID, GenreID) values(?,?);");
			pstmt.setInt(1, ID);
	...
	public static void updateProduct(String ToUpdate, String NewValue, String Title) throws SQLException{
		pstmt=conn.prepareStatement("UPDATE gggameshopv2v2.games SET "+ToUpdate+"=? WHERE Title=?;");
